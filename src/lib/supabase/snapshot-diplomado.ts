...
const MANIFEST_PATH = "diplomado/active/manifest.json";
const LOGS_PATH = "diplomado/logs/upload-log.json";
const DEFAULT_CHUNK_SIZE = 20_000;
const TEMP_UPLOAD_PREFIX = "diplomado/uploads";
...
function isNotFoundError(error: unknown) {
  ...
  const anyErr = error as { statusCode?: number; message?: string; name?: string; originalError?: unknown };
  if (anyErr.statusCode === 404) return true;
  const message = String(anyErr.message ?? "");
  if (anyErr.name === "StorageUnknownError" && (!anyErr.originalError || Object.keys(anyErr.originalError as object).length === 0)) return true;
...
async function downloadJSONOptional<T>(
  ...
  while (true) {
    ...
    if (isNotFoundError(error)) {
      if (attempt < retries) { attempt++; await sleep(150 * 2 ** (attempt - 1)); continue; }
      return null;
    }
    if (attempt < retries) { attempt++; await sleep(150 * 2 ** (attempt - 1)); continue; }
...
async function removeFilesOptional(supabase: ReturnType<typeof getSupabaseServerClient>, paths: string[]) {
...
function buildChunkPath(version: string, chunkIndex: number) {
  return `diplomado/versions/${version}/chunk-${String(chunkIndex).padStart(5, "0")}.json`;
}

async function readChunk(supabase: ReturnType<typeof getSupabaseServerClient>, path: string): Promise<DataRow[]> {
  return (await downloadJSONOptional<DataRow[]>(supabase, path, { retries: 2 })) ?? [];
}

async function readManifest(supabase: ReturnType<typeof getSupabaseServerClient>): Promise<StoredDatasetManifest | null> {
...
async function listFolderPaths(supabase: ReturnType<typeof getSupabaseServerClient>, folder: string) {
  const { data, error } = await supabase.storage.from(BUCKET).list(folder, {
    limit: 1_000,
    sortBy: { column: "name", order: "asc" },
  });
  if (error) { console.warn("Storage list failed:", error.message); return []; }
  return (data ?? []).filter((e) => e.name !== ".emptyFolderPlaceholder").map((e) => `${folder}/${e.name}`);
}

export async function getActiveDiplomadoSnapshot(): Promise<Dataset | null> {
  try {
    ...
    if (manifest) {
      ...
      for (const chunkPath of manifest.chunkPaths) {
        rows.push(...(await readChunk(supabase, chunkPath)));
      }
      return { meta: manifest.meta, rows };
    }
    return null;
  } catch (error) {
    console.warn("getActiveDiplomadoSnapshot failed:", error);
    return null;
  }
}

export function getDiplomadoTempUploadChunkSize() {
  return TEMP_UPLOAD_CHUNK_SIZE;
}

export async function beginDiplomadoTempUpload(fileName: string) {
  const supabase = getSupabaseServerClient();
  await ensureBucket(supabase);
  const uploadId = crypto.randomUUID();
  const session: TempUploadSession = { uploadId, fileName, createdAtISO: new Date().toISOString() };
  await uploadJSON(supabase, buildTempUploadSessionPath(uploadId), session);
  return { uploadId, chunkSize: TEMP_UPLOAD_CHUNK_SIZE };
}

export async function storeDiplomadoTempUploadChunk(uploadId: string, partNumber: number, chunk: Uint8Array) {
  const supabase = getSupabaseServerClient();
  await ensureBucket(supabase);
  await uploadBlob(
    supabase,
...
export async function assembleDiplomadoTempUploadFile(uploadId: string, fileName: string, totalChunks: number) {
  const supabase = getSupabaseServerClient();
  const session = await downloadJSONOptional<TempUploadSession>(supabase, buildTempUploadSessionPath(uploadId), { retries: 1 });
  if (!session) throw new Error("No existe una sesion de carga activa para este archivo.");
  const parts = await Promise.all(
    Array.from({ length: totalChunks }, async (_, partNumber) => {
      ...
      if (error) throw new Error(`Falta el bloque ${partNumber + 1} del archivo temporal.`);
...
export async function cleanupDiplomadoTempUpload(uploadId: string) {
  const supabase = getSupabaseServerClient();
...
export async function beginDiplomadoSnapshotWrite(meta: {
  sourceFileName: string;
  sheetName: string;
  importedAtISO?: string;
  append?: boolean;
...
export async function appendDiplomadoSnapshotChunk(session: SnapshotWriteSession, rows: DataRow[]) {
...
export async function finalizeDiplomadoSnapshotWrite(session: SnapshotWriteSession): Promise<{ meta: DatasetMeta }> {
  ...
  try {
    const logs = (await downloadJSONOptional<UploadLogEntry[]>(session.supabase, LOGS_PATH)) || [];
    logs.push({ timestamp: new Date().toISOString(), fileName: nextMeta.sourceFileName, sheetName: nextMeta.sheetName, rowCount: nextMeta.rowCount, mode: "replace", totalRowsAfter: nextMeta.rowCount });
    await uploadJSON(session.supabase, LOGS_PATH, logs);
  } catch { /* non-critical */ }
  return { meta: nextMeta };
}

export async function abortDiplomadoSnapshotWrite(session: SnapshotWriteSession) {
