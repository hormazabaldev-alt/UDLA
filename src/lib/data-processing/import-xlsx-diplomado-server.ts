...
function selectBestSheet(workbook: XLSX.WorkBook): SheetCandidate | null {
  const candidates = workbook.SheetNames.map((sheetName) => {
    ...
    return { sheetName, sheet, range: info.range, headers: info.headers, ...scored, rowCount: info.rowCount };
  }).filter((c): c is SheetCandidate => c !== null);
  return candidates.sort((a, b) => b.score - a.score)[0] ?? null;
}

function readSheetChunk(candidate: SheetCandidate, startRow: number, endRow: number) {
  const range = XLSX.utils.encode_range({ s: { r: startRow, c: candidate.range.s.c }, e: { r: endRow, c: candidate.range.e.c } });
  return XLSX.utils.sheet_to_json<Record<string, unknown>>(candidate.sheet, { defval: null, raw: true, blankrows: false, range, header: candidate.headers });
}
...
async function emitProcessingProgress(processedRows: number, totalRows: number, uploadedChunks: number, onProgress?: (e: ImportProgressEvent) => Promise<void> | void) {
  await onProgress?.({ stage: "processing", message: `Procesando ${Math.min(totalRows, processedRows)} de ${totalRows} filas...`, processedRows: Math.min(totalRows, processedRows), totalRows, uploadedChunks, totalChunks: Math.max(1, Math.ceil(totalRows / STORAGE_CHUNK_SIZE)) });
}

export async function importXlsxDiplomadoSnapshot(
  file: File,
  ...
  await onProgress?.({ stage: "analyzing", message: "Analizando hojas del Excel..." });

  const best = selectBestSheet(workbook);
  if (!best) return { ok: false, issues: [{ message: "No se encontró ninguna hoja en el Excel." }], preview: [] };
  if (best.rowCount === 0) return { ok: false, issues: [{ message: "El archivo no contiene filas de datos." }], preview: [] };
  if (best.missing.length > 0) return { ok: false, issues: best.missing.map((column) => ({ column, message: `Falta la columna requerida: ${column}` })), preview: [] };
  ...
  try {
    session = await beginDiplomadoSnapshotWrite({ sourceFileName: file.name, sheetName: best.sheetName, append: opts?.append });
    ...
    for (let startRow = best.range.s.r + 1; startRow <= best.range.e.r; startRow += PROCESS_BATCH_SIZE) {
      ...
      for (let offset = 0; offset < rawRows.length; offset++) {
        ...
        if (absoluteProcessedRows >= nextProgressAt) {
          await emitProcessingProgress(absoluteProcessedRows, best.rowCount, session.chunkPaths.length, onProgress);
          nextProgressAt += PROGRESS_STEP;
        }
        if (storageBuffer.length >= STORAGE_CHUNK_SIZE) {
          await flushChunk(session, storageBuffer, { stage: "processing", message: `Procesando ${Math.min(best.rowCount, rowIndex + 1)} de ${best.rowCount} filas...`, processedRows: Math.min(best.rowCount, rowIndex + 1), totalRows: best.rowCount }, onProgress);
        }
      }
    ...
    await flushChunk(session, storageBuffer, { stage: "persisting", message: "Guardando bloques finales...", processedRows: best.rowCount, totalRows: best.rowCount }, onProgress);
    ...
    await onProgress?.({ stage: "completed", message: `Importación completada: ${finalized.meta.rowCount} filas.`, processedRows: best.rowCount, totalRows: best.rowCount, uploadedChunks: session.chunkPaths.length, totalChunks: session.chunkPaths.length });
