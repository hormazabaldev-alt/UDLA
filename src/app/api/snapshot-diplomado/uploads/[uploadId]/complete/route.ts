...
import { assembleDiplomadoTempUploadFile, cleanupDiplomadoTempUpload } from "@/lib/supabase/snapshot-diplomado";
import { assertDashboardAdmin } from "@/lib/server/dashboard-admin";

export const runtime = "nodejs";
export const maxDuration = 300;

export async function POST(req: Request, context: { params: Promise<{ uploadId: string }> }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const file = await assembleDiplomadoTempUploadFile(uploadId, fileName, totalChunks);
        ...
        if (uploadId) await cleanupDiplomadoTempUpload(uploadId);
        controller.close();
      }
    },
  });
